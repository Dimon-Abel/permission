...
namespace GoldCloud.Infrastructure.Shared.Helper
{
    /// <summary>
    /// 雪花算法
    /// </summary>
    public class SnowflakeTool
    {
        ...
        public SnowflakeTool(long workerId)
        {
            if (workerId > maxWorkerId || workerId < 0)
                throw new System.Exception(string.Format("节点id 不能大于 {0} 或者 小于 0 ", workerId));
        ...
        public long nextId()
        {
            lock (this)
            {
                ...
                { //如果当前时间戳比上一次生成ID时时间戳还小，抛出异常，因为不能保证现在生成的ID之前没有生成过
                    throw new System.Exception(string.Format("Clock moved backwards.  Refusing to generate id for {0} milliseconds",
